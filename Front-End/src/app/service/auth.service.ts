...
export class AuthService {
    ...
    login(data: { email: string; password: string }) {
        console.log(data);
        return this.http
            .post<AuthData>(`${environment.apiUrl}auth/login`, data)
            .pipe(
                tap(async (user) => {
    ...
    uploadAvatar(data: FormData, id: number) {
        return this.http
            .patch<User>(
                `${environment.apiUrl}users/${id}/avatar`,
                data
            )
            .pipe(
                tap((response) => {
                    const userData = this.authSub.getValue();
                    if (userData) {
    ...
    private autoLogout(data: AuthData) {
        const dataExp = this.jwtHelper.getTokenExpirationDate(
            data.accessToken
        ) as Date;
